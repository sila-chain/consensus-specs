# SilaDeneb -- Networking

<!-- mdformat-toc start --slug=github --no-anchors --maxlevel=6 --minlevel=2 -->

- [Introduction](#introduction)
- [Modifications in SilaDeneb](#modifications-in-sila_deneb)
  - [Preset](#preset)
...
This document contains the consensus-layer networking specifications for SilaDeneb.

The specification of these changes continues in the same format as the network
specifications of previous upgrades, and assumes them as pre-requisite.

## Modifications in SilaDeneb

### Preset

*[New in SilaDeneb:SIP4844]*
...
| Name                                    | Value                    | Description                                                    |
| --------------------------------------- | ------------------------ | -------------------------------------------------------------- |
| `MAX_REQUEST_BLOCKS_SILA_DENEB`              | `2**7` (= 128)           | Maximum number of blocks in a single request                   |
...
def compute_max_request_blob_sidecars() -> uint64:
    ...
    """
    return uint64(MAX_REQUEST_BLOCKS_SILA_DENEB * MAX_BLOBS_PER_BLOCK)
```
...
Some gossip meshes are upgraded in SilaDeneb to support upgraded types.
...
`ATTESTATION_PROPAGATION_SLOT_RANGE` slots, for SIP-7045.
...
*[Modified in SilaDeneb:SIP7045]* Attestations from the previous epoch are now
...
| ------------------------------ | ------------------- |
| `SILA_DENEB_FORK_VERSION` and later | `sila_deneb.BlobSidecar` |
...
```
(
  List[Root, MAX_REQUEST_BLOCKS_SILA_DENEB]
)
```
...
`[max(current_epoch - MIN_EPOCHS_FOR_BLOB_SIDECARS_REQUESTS, SILA_DENEB_FORK_EPOCH), current_epoch]`.
...
`minimum_request_epoch = max(current_epoch - MIN_EPOCHS_FOR_BLOB_SIDECARS_REQUESTS, SILA_DENEB_FORK_EPOCH)`.
