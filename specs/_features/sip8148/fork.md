...
- [Fork to SIP-8148](#fork-to-sip-8148)

<!-- mdformat-toc end -->

## Introduction

This document describes the process of the SIP-8148 upgrade.
...
| Name                   | Value                                 |
| ---------------------- | ------------------------------------- |
| `SIP8148_FORK_VERSION` | `Version('0xe8148000')`               |
| `SIP8148_FORK_EPOCH`   | `Epoch(18446744073709551615)` **TBD** |

## Fork to SIP-8148

If `state.slot % SLOTS_PER_EPOCH == 0` and
`compute_epoch_at_slot(state.slot) == SIP8148_FORK_EPOCH`, an irregular state
change is made to upgrade to SIP-8148.

The upgrade occurs after the completion of the inner loop of `process_slots`
that sets `state.slot` equal to `SIP8148_FORK_EPOCH * SLOTS_PER_EPOCH`.

```python
def upgrade_to_sip8148(pre: heze.BeaconState) -> BeaconState:
    ...
    post = BeaconState(
        ...
        fork=Fork(
            ...
            # [New in SIP8148]
            current_version=SIP8148_FORK_VERSION,
            epoch=epoch,
        ),
