...
- [Fork to sip6800](#fork-to-sip6800)
...
This document describes the process of the sip6800 upgrade.
...
| Name                   | Value                                 |
| ---------------------- | ------------------------------------- |
| `SIP6800_FORK_VERSION` | `Version('0x05000000')`               |
| `SIP6800_FORK_EPOCH`   | `Epoch(18446744073709551615)` **TBD** |

## Fork to sip6800

### Fork trigger

The fork is triggered at epoch `SIP6800_FORK_EPOCH`.

*Note*: For the pure sip6800 networks, the `upgrade_to_sip6800` function is
applied to transition the genesis state to this fork.

### Upgrading the state

If `state.slot % SLOTS_PER_EPOCH == 0` and
`compute_epoch_at_slot(state.slot) == SIP6800_FORK_EPOCH`, an irregular state
change is made to upgrade to sip6800.

The upgrade occurs after the completion of the inner loop of `process_slots`
that sets `state.slot` equal to `SIP6800_FORK_EPOCH * SLOTS_PER_EPOCH`. Care
...
```python
def upgrade_to_sip6800(pre: deneb.BeaconState) -> BeaconState:
    ...
    post = BeaconState(
        ...
        fork=Fork(
            ...
            # [Modified in SIP6800]
            current_version=SIP6800_FORK_VERSION,
            epoch=epoch,
        ),
