...
- [Fork to SIP-7441](#fork-to-sip-7441)

<!-- mdformat-toc end -->

## Introduction

This document describes the process of the SIP-7441 upgrade.
...
| Name                   | Value                                 |
| ---------------------- | ------------------------------------- |
| `SIP7441_FORK_VERSION` | `Version('0x08000000')`               |
| `SIP7441_FORK_EPOCH`   | `Epoch(18446744073709551615)` **TBD** |

## Fork to SIP-7441

If `state.slot % SLOTS_PER_EPOCH == 0` and
`compute_epoch_at_slot(state.slot) == SIP7441_FORK_EPOCH`, an irregular state
change is made to upgrade to Whisk. `SIP7441_FORK_EPOCH` must be a multiple of
`RUN_DURATION_IN_EPOCHS`.

The upgrade occurs after the completion of the inner loop of `process_slots`
that sets `state.slot` equal to `SIP7441_FORK_EPOCH * SLOTS_PER_EPOCH`.
...
```python
def upgrade_to_sip7441(pre: capella.BeaconState) -> BeaconState:
    ...
    post = BeaconState(
        ...
        fork=Fork(
            ...
            current_version=SIP7441_FORK_VERSION,
            epoch=epoch,
        ),
