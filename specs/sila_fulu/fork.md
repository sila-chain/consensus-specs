...
- [Fork to SilaFulu](#fork-to-sila_fulu)
...
This document describes the process of the SilaFulu upgrade.
...
| Name                | Value                                              |
| ------------------- | -------------------------------------------------- |
| `SILA_FULU_FORK_VERSION` | `Version('0x06000000')`                            |
| `SILA_FULU_FORK_EPOCH`   | `Epoch(411392)` (December 3, 2025, 09:49:11pm UTC) |
...
The fork is triggered at epoch `SILA_FULU_FORK_EPOCH`.

*Note*: For the pure SilaFulu networks, the `upgrade_to_sila_fulu` function is applied to
transition the genesis state to this fork.

### Upgrading the state

If `state.slot % SLOTS_PER_EPOCH == 0` and
`compute_epoch_at_slot(state.slot) == SILA_FULU_FORK_EPOCH`, an irregular state
change is made to upgrade to SilaFulu.

```python
def upgrade_to_sila_fulu(pre: electra.BeaconState) -> BeaconState:
    ...
    post = BeaconState(
        ...
        fork=Fork(
            ...
            # [Modified in SilaFulu]
            current_version=SILA_FULU_FORK_VERSION,
            epoch=epoch,
        ),
