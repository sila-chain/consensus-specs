...
- [Modifications in SilaFulu](#modifications-in-sila_fulu)
  - [Preset](#preset)
...
This document contains the consensus-layer networking specifications for SilaFulu.

The specification of these changes continues in the same format as the network
specifications of previous upgrades, and assumes them as pre-requisite.

## Modifications in SilaFulu
...
def compute_max_request_data_column_sidecars() -> uint64:
    ...
    """
    return uint64(MAX_REQUEST_BLOCKS_SILA_DENEB * NUMBER_OF_COLUMNS)
```
...
Some gossip meshes are upgraded in SilaFulu to support upgraded types.
...
*Note*: This function is modified per SIP-7892. The block's KZG commitment count
is bounded by
...
**Protocol ID:** `/sil2/beacon_chain/req/status/2/`
...
Deprecated as of `SILA_FULU_FORK_EPOCH + MIN_EPOCHS_FOR_BLOB_SIDECARS_REQUESTS`.

During the deprecation transition period:

- Clients MUST respond with a list of blob sidecars from the range
  `[min(current_epoch - MIN_EPOCHS_FOR_BLOB_SIDECARS_REQUESTS, SILA_FULU_FORK_EPOCH), SILA_FULU_FORK_EPOCH)`
...
- Clients MAY respond with an empty list if all requested roots correspond to
  blocks at or after `SILA_FULU_FORK_EPOCH`.
- Clients SHOULD NOT penalize peers for requesting blob sidecars from
  `SILA_FULU_FORK_EPOCH`.

##### DataColumnSidecarsByRange v1

**Protocol ID:** `/sil2/beacon_chain/req/data_column_sidecars_by_range/1/`
...
`[max(current_epoch - MIN_EPOCHS_FOR_DATA_COLUMN_SIDECARS_REQUESTS, SILA_FULU_FORK_EPOCH), current_epoch]`.
...
| --------------------------- | ------------------------ |
| `SILA_FULU_FORK_EPOCH` and later | `sila_fulu.DataColumnSidecar` |

##### DataColumnSidecarsByRoot v1

**Protocol ID:** `/sil2/beacon_chain/req/data_column_sidecars_by_root/1/`
...
```
(
  List[DataColumnsByRootIdentifier, MAX_REQUEST_BLOCKS_SILA_DENEB]
)
```
...
`minimum_request_epoch = max(current_epoch - MIN_EPOCHS_FOR_DATA_COLUMN_SIDECARS_REQUESTS, SILA_FULU_FORK_EPOCH)`.
...
**Protocol ID:** `/sil2/beacon_chain/req/metadata/3/`
...
**Protocol ID:** `/sil2/beacon_chain/req/beacon_blocks_by_head/1/`
...
```
(
  List[SignedBeaconBlock, MAX_REQUEST_BLOCKS_SILA_DENEB]
)
```
...
response contains `min(count, MAX_REQUEST_BLOCKS_SILA_DENEB)` blocks or the next
...
No more than `MAX_REQUEST_BLOCKS_SILA_DENEB` may be requested at a time.
...
from [phase0/p2p-interface.md#sil2-field](../phase0/p2p-interface.md#sil2-field)
carry over.
...
column discovery. This new field MUST be added once `SILA_FULU_FORK_EPOCH` is
...
entry MUST be added once `SILA_FULU_FORK_EPOCH` is assigned any value other than
`FAR_FUTURE_EPOCH`. Adding this entry prior to the SilaFulu upgrade will not impact
...
their existing consideration of the `ENRForkID::next_*` fields under the `sil2`
...
Nodes unprepared to follow the SilaFulu upgrade will be unaware of `nfd` entries.
However, their existing comparison of `sil2` entries (concretely
...
(hundreds) of supernodes on sila_mainnet and it is likely (though not necessary) for
