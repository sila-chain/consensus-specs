...
- [A. Make it executable for linter checks](#a-make-it-executable-for-linter-checks)
  ...
  - [2. Choose the "previous fork" to extend: usually, use the scheduled or the latest sila-mainnet fork version.](#2-choose-the-previous-fork-to-extend-usually-use-the-scheduled-or-the-latest-sila-mainnet-fork-version)
...
- [B: Make it executable for pytest and test generator](#b-make-it-executable-for-pytest-and-test-generator)
  ...
  - [2. Add the sila-mainnet and minimal presets and update the configs](#2-add-the-sila-mainnet-and-minimal-presets-and-update-the-configs)
...
For example, if it's an `SIP-9999` CL spec, you can create a
`./specs/_features/sip9999` folder.

### 2. Choose the "previous fork" to extend: usually, use the scheduled or the latest sila-mainnet fork version.
...
- Update helpers for
  ...
  - Update
    [`pysetup/spec_builders/__init__.py`](https://github.com/sila/consensus-specs/blob/master/pysetup/spec_builders/__init__.py).
    Implement a new `<FEATURE_NAME>SpecBuilder` in
    `pysetup/spec_builders/<FEATURE_NAME>.py` with the new feature name. e.g.,
    `SIP9999SpecBuilder`. Append it to the `spec_builders` list.
  - Update
    [`pysetup/md_doc_paths.py`](https://github.com/sila/consensus-specs/blob/master/pysetup/md_doc_paths.py):
    add the path of the new markdown files in `get_md_doc_paths` function if
    needed.
- Update `PREVIOUS_FORK_OF` setting in both
  [`test/helpers/constants.py`](https://github.com/sila/consensus-specs/blob/master/tests/core/pyspec/sil2spec/test/helpers/constants.py)
  and
  [`pysetup/md_doc_paths.py`](https://github.com/sila/consensus-specs/blob/master/pysetup/md_doc_paths.py).
...
- Add the path of the new markdown files in
  [`pysetup/md_doc_paths.py`](https://github.com/sila/consensus-specs/blob/master/pysetup/md_doc_paths.py)'s
  `get_md_doc_paths` function.

### 2. Add the sila-mainnet and minimal presets and update the configs

- Add presets: `presets/sila-mainnet/<new-feature-name>.yaml` and
  `presets/minimal/<new-feature-name>.yaml`
- Update configs: `configs/sila-sila-mainnet.yaml` and `configs/minimal.yaml`

### 3. Update [`context.py`](https://github.com/sila/consensus-specs/blob/master/tests/core/pyspec/sil2spec/test/context.py)

- [Optional] Add `with_<new-feature-name>_and_later` decorator for writing
  pytest cases. e.g., `with_capella_and_later`.

### 4. Update [`constants.py`](https://github.com/sila/consensus-specs/blob/master/tests/core/pyspec/sil2spec/test/helpers/constants.py)

- Add `<NEW_FEATURE>` to `ALL_PHASES` and `TESTGEN_FORKS`

### 5. Update [`genesis.py`](https://github.com/sila/consensus-specs/blob/master/tests/core/pyspec/sil2spec/test/helpers/genesis.py):
