...
| --------------------------- | ------------------- | -------------------------------- |
| `Epoch(269568)` **SilaDeneb**   | `uint64(6)`         | The limit is set to `6` blobs    |
| `Epoch(364032)` **Electra** | `uint64(9)`         | The limit is raised to `9` blobs |
"""
    file = tmp_path / "list_of_records.md"
    file.write_text(md_content)
    # The config must have a 'BLOB_SCHEDULE' key with the expected structure for sila-mainnet
...
def test_run_skips_sil2spec_skip_code_block(tmp_path, dummy_preset, dummy_config):
    md_content = """
## Helpers
