...
@with_presets([SILA_MAINNET], reason="too few validators with minimal config")
@spec_state_test
...
@with_capella_and_later
@with_presets([MINIMAL], reason="not enough validators with sila_mainnet config")
@spec_state_test
...
def test_partially_withdrawable_validator_legacy_exact_max(spec, state):
    ...
    validator_index = 0
    set_sil1_withdrawal_credential_with_balance(spec, state, validator_index)
    assert not check_is_partially_withdrawable_validator(spec, state, validator_index)
