...
from sil_consensus_specs.test.helpers.bls_to_execution_changes import get_signed_address_change
...
from sil_consensus_specs.test.helpers.forks import is_post_sip8148, is_post_electra, is_post_gloas
...
def test_deposit_and_bls_change(spec, state):
    ...
    assert validator_credentials[:1] == spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX
...
def test_top_up_to_fully_withdrawn_validator(spec, state):
    ...
    # With sila_mainnet preset, it holds
...
def _insert_validator(spec, state, balance):
    ...
    validator = spec.Validator(
        ...
        withdrawal_credentials=spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + b"\x00" * 11 + b"\x56" * 20,
...
@with_capella_and_later
@with_presets([MINIMAL], reason="too many validators with sila_mainnet config")
@spec_state_test
