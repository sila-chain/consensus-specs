from sil_consensus_specs.test.bellatrix.block_processing.test_process_voluntary_exit import (
...
def test_invalid_voluntary_exit_with_current_fork_version_not_is_before_fork_epoch(spec, state):
    """
    Since SilaDeneb, the VoluntaryExit domain is fixed to `CAPELLA_FORK_VERSION`
    """
...
def test_voluntary_exit_with_previous_fork_version_not_is_before_fork_epoch(spec, state):
    ...
    Note: This test is valid for ``spec.fork == SILA_DENEB`` and invalid for subsequent forks
    """
    assert state.fork.previous_version != state.fork.current_version

    if spec.fork == SILA_DENEB:
