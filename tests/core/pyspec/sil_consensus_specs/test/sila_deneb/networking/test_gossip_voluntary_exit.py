...
from sil_consensus_specs.test.helpers.gossip import get_filename, get_seen, run_validate_gossip
...
@spec_state_test
@always_bls
def test_gossip_voluntary_exit__reject_sila_deneb_signature(spec, state):
    """
    Test that a voluntary exit signed under SILA_DENEB_FORK_VERSION (i.e. not pinned
    to CAPELLA_FORK_VERSION per SIP-7044) is rejected.
    """
    yield "topic", "meta", "voluntary_exit"
    ...
    # Sign with SILA_DENEB fork version (the wrong domain under SIP-7044).
    signed_exit = create_signed_voluntary_exit(
        spec, state, validator_index=0, fork_version=spec.config.SILA_DENEB_FORK_VERSION
    )
    yield get_filename(signed_exit), signed_exit
