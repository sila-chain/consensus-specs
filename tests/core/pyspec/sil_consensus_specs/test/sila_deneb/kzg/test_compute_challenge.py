...
from sil_utils.hexadecimal import encode_hex

from sil_consensus_specs.test.context import only_generator, single_phase, spec_test, with_phases
from sil_consensus_specs.test.helpers.constants import SILA_DENEB
from sil_consensus_specs.test.utils.kzg_tests import VALID_BLOBS
...
def _compute_challenge_case_valid(blob_index):
    ...
    @manifest(preset_name="general", suite_name="kzg-sila_mainnet")
    @only_generator("too slow")
    @with_phases([SILA_DENEB])
    @spec_test
    @single_phase
    def the_test(spec):
