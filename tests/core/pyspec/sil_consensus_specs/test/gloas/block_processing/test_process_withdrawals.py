...
from sil_consensus_specs.test.helpers.builders import add_builder_to_registry
from sil_consensus_specs.test.helpers.withdrawals import (
...
def test_single_builder_withdrawal(spec, state):
    ...
    Input State Configured:
        ...
        - builder_pending_withdrawals[0].amount: 1 SIL
...
def test_multiple_builder_withdrawals(spec, state):
    ...
    Input State Configured:
        ...
        - builder_pending_withdrawals[*].amount: 0.5 SIL each
        - builders[0,1,2].balance: >= amount

    Output State Verified:
        - payload_expected_withdrawals: Contains 3 builder withdrawals
        - builders[0,1,2].balance: Each decreased by 0.5 SIL
...
def test_builder_withdrawal_insufficient_balance(spec, state):
    ...
    Input State Configured:
        - state.builders[0]: Builder exists with only 1 SIL balance
        - builder_pending_withdrawals: Contains 1 entry requesting 5 SIL
        - builders[0].balance: 1 SIL (insufficient for requested 5 SIL)

    Output State Verified:
        - payload_expected_withdrawals: Contains 1 withdrawal
        - withdrawal.amount: 5 SIL (requested amount)
...
def test_pending_withdrawals_processing(spec, state):
    ...
    Input State Configured:
        ...
        - balances[*]: MAX_EFFECTIVE_BALANCE + 1 SIL (excess to withdraw)
...
def test_pending_withdrawals_processing_exceeds_limit(spec, state):
    ...
    Output State Verified:
        ...
        - balances[MAX..MAX+1]: MAX_EFFECTIVE_BALANCE + 1 SIL (unchanged)
        - next_withdrawal_index: Incremented by MAX_PENDING_PARTIALS_PER_WITHDRAWALS_SWEEP
    """
...
def test_compounding_validator_partial_withdrawal(spec, state):
    ...
    Input State Configured:
        ...
        - balances[0]: MAX_EFFECTIVE_BALANCE_ELECTRA + 1 SIL (excess)
...
def test_builder_payments_exceed_limit_blocks_other_withdrawals(spec, state):
    ...
    capped_validator_balances = {
        ...
        if state.validators[i].withdrawal_credentials[0:1] == spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX
    }
...
def test_full_builder_payload_reserves_sweep_slot(spec, state):
    ...
    This test verifies the fix from https://github.com/sila-chain/consensus-specs/pull/4832
    ...
    Previous Bug (before the fix):
        ...
        See also: https://github.com/sila-chain/consensus-specs/pull/4835
    ...
    for i, validator in enumerate(state.validators):
        if validator.withdrawal_credentials[0:1] == spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX:
...
def test_single_builder_sweep_withdrawal(spec, state):
    ...
    Output State Verified:
        ...
        - withdrawal.amount: 5 SIL (full balance)
...
def test_multiple_builder_sweep_withdrawals(spec, state):
    ...
    Input State Configured:
        ...
        - builders[0,1,2].balance: 1, 2, 3 SIL respectively
        - builder_pending_withdrawals: Empty
        - next_withdrawal_builder_index: 0
    ...
    Output State Verified:
        ...
        - withdrawal amounts: 1, 2, 3 SIL respectively
    """
    builder_indices = [0, 1, 2]
    balances = {
...
def test_builder_sweep_zero_balance_skipped(spec, state):
    ...
    Input State Configured:
        ...
        - builders[1].balance: 2 SIL (should be processed)
        - next_withdrawal_builder_index: 0
...
def test_builder_sweep_not_withdrawable_skipped(spec, state):
    ...
    Input State Configured:
        ...
        - builders[0].balance: 5 SIL
        - builders[1].withdrawable_epoch: current_epoch (eligible)
        - builders[1].balance: 2 SIL
        - next_withdrawal_builder_index: 0

    Output State Verified:
        - payload_expected_withdrawals: 1 withdrawal (only builder 1)
        - builders[0].balance: 5 SIL (unchanged, not withdrawable yet)
        - builders[1].balance: 0 (withdrawn)
    """
    prepare_process_withdrawals(
        spec,
...
def test_empty_parent_preserves_populated_expected_withdrawals(spec, state):
    ...
    Input State Configured:
        - state.builders[0]: Builder with pending withdrawal (1 SIL)
