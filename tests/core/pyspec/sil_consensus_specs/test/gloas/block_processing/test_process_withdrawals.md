...
| `state.balances[*]`                                      | `Gwei`                                                              | `uint64` in [0, 2^64-1]. **Same length as** `validators[]`. Must have excess over `MIN_ACTIVATION_BALANCE` (32 SIL) for partial withdrawals.                         | Check available balance                 |
...
| (in `validators`) `[*].effective_balance`                | `Gwei`                                                              | `uint64` in [0, 2048 SIL]. **Constraint**: partial withdrawals require `>= MIN_ACTIVATION_BALANCE` (32 SIL).                                                         | Check sufficient balance                |
...
| (in `validators`) `[*].withdrawal_credentials`           | `Bytes32`                                                           | `Bytes32`. **Prefix byte**: `0x01`=SIL1, `0x02`=compounding. **Bytes [12:32]** = execution address. Validators need `0x01`/`0x02` prefix for withdrawal eligibility. | Extract withdrawal address              |
...
| `MIN_ACTIVATION_BALANCE`                     | 32 SIL (32x10^9 Gwei) | Minimum balance for active validator  |
