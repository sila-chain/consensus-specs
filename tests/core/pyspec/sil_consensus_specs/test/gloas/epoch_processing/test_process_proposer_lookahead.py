from sil_consensus_specs.test.context import spec_state_test, with_gloas_and_later
...
def test_proposer_lookahead_does_not_contain_slashed_validators(spec, state):
    """
    [SIP-8045] The newly appended epoch of ``proposer_lookahead`` must not
...
def test_proposer_lookahead_full_with_many_slashed_validators(spec, state):
    """
    [SIP-8045] The lookahead must still fill its full ``SLOTS_PER_EPOCH`` entries
