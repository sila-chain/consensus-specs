...
from sil_consensus_specs.test.gloas.block_processing.test_process_payload_attestation import (
...
from sil_consensus_specs.test.helpers.execution_requests import (
    get_non_empty_execution_requests,
)
from sil_consensus_specs.test.helpers.keys import builder_privkeys, privkeys
...
def test_proposer_lookahead_excludes_slashed_validators(spec, state):
    ...
    # SilaFulu includes slashed validators
    proposers_in_sila_fulu = spec.sila_fulu.get_beacon_proposer_indices(state, new_half_epoch)
    assert any(state.validators[v].slashed for v in proposers_in_sila_fulu)
