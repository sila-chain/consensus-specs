...
def _setup_switch_to_compounding_validator(spec, state, validator_index):
    """
    Set up a validator with SIL1 withdrawal credentials and balance above
...
def _run_epoch_boundary_full_parent(spec, state, gap_epochs):
    ...
    assert spec.has_sil1_withdrawal_credential(state.validators[consolidation_validator_index])
...
def test_switch_to_compounding_across_epoch_boundary(spec, state):
    ...
    assert spec.has_sil1_withdrawal_credential(state.validators[validator_index])
...
def test_epoch_boundary_full_parent_all_requests_gap_5_epochs(spec, state):
    ...
    for exit_index in exit_validator_indices:
        assert spec.has_sil1_withdrawal_credential(state.validators[exit_index])
