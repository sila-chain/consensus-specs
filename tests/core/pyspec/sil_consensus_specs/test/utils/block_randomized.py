...
This module generates randomized test scenarios for Sila consensus layer
...
How to run (from project root):
    ...
    .venv/bin/python -m sil_consensus_specs.test.utils.block_randomized

    # Then format with ruff
    .venv/bin/ruff check --fix tests/core/pyspec/sil_consensus_specs/test/*/random/test_random.py
    .venv/bin/ruff format tests/core/pyspec/sil_consensus_specs/test/*/random/test_random.py

    # Generate for specific fork only
    .venv/bin/python -m sil_consensus_specs.test.utils.block_randomized --fork phase0

    # Print to stdout instead of writing files
    .venv/bin/python -m sil_consensus_specs.test.utils.block_randomized --stdout

    # List available forks
    .venv/bin/python -m sil_consensus_specs.test.utils.block_randomized --list-forks
"""
...
from sil_consensus_specs.test.utils.randomized_block_tests import (
    epoch_transition,
    last_slot_in_epoch,
    no_block,
    no_op_validation,
    ...
    random_block_sila_deneb,
    random_block_electra,
    random_block_sila_fulu,
    random_block_gloas,
    ...
    randomize_state_sila_deneb,
    randomize_state_electra,
    randomize_state_sila_fulu,
...
DEFAULT_OUTPUT_DIR: str = "tests/core/pyspec/sil_consensus_specs/test"
...
from sil_consensus_specs.test.helpers.constants import {phase}
...
class RandomizedTestGenerator:
    ...
    FORK_CONFIGS: ClassVar[dict[str, ForkConfig]] = {
        ...
        SILA_DENEB: ForkConfig(
            name=SILA_DENEB,
            state_randomizer=randomize_state_sila_deneb,
            block_randomizer=random_block_sila_deneb,
        ),
        ELECTRA: ForkConfig(
            name=ELECTRA,
        ...
        SILA_FULU: ForkConfig(
            name=SILA_FULU,
            state_randomizer=randomize_state_sila_fulu,
            block_randomizer=random_block_sila_fulu,
        ),
        GLOAS: ForkConfig(
            name=GLOAS,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate randomized block tests for Sila consensus layer."
    )
    parser.add_argument(
        "--fork",
        type=str,
