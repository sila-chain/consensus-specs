...
from sil_consensus_specs.test.helpers.blob import (
    get_sample_blob_tx,
)
...
from sil_consensus_specs.test.helpers.forks import is_post_sila_fulu
from sil_consensus_specs.test.helpers.genesis import build_mock_builder
from sil_consensus_specs.test.helpers.inactivity_scores import (
    randomize_inactivity_scores,
)
from sil_consensus_specs.test.helpers.keys import builder_privkeys
...
from sil_consensus_specs.test.helpers.payload_attestation import get_random_payload_attestations
from sil_consensus_specs.test.helpers.random import (
...
def randomize_state_sila_deneb(spec, state, stats, exit_fraction=0.1, slash_fraction=0.1):
...
def randomize_state_electra(spec, state, stats, exit_fraction=0.1, slash_fraction=0.1):
    scenario_state = randomize_state_sila_deneb(
        spec,
        state,
        stats,
...
def randomize_state_sila_fulu(spec, state, stats, exit_fraction=0.1, slash_fraction=0.1):
...
def randomize_state_gloas(spec, state, stats, exit_fraction=0.1, slash_fraction=0.1):
    scenario_state = randomize_state_sila_fulu(
        spec,
        state,
        stats,
        exit_fraction=exit_fraction,
...
def random_block_sila_deneb(spec, state, signed_blocks, scenario_state, rng=None):
    if rng is None:
        rng = Random(3456)
    ...
    # TODO: add MAX_BLOBS_PER_BLOCK_SILA_FULU at sila_fulu
...
def random_block_electra(spec, state, signed_blocks, scenario_state, rng=None):
    if rng is None:
        rng = Random(3456)
    block = random_block_sila_deneb(spec, state, signed_blocks, scenario_state, rng=rng)
...
def random_block_sila_fulu(spec, state, signed_blocks, scenario_state, rng=None):
    if rng is None:
        rng = Random(3456)
