...
from sil_consensus_specs.test.helpers.deposit_requests import (
...
def test_process_deposit_request_extra_gwei(spec, state):
    """The deposit amount must be at least 1 SIL and must be a multiple of gwei."""
    validator_index = len(state.validators)
...
@with_electra_and_later
@spec_state_test
def test_process_deposit_request_sil1_credentials(spec, state):
    """
    Test deposit with SIL1 credentials (0x01) in validator path.

    Input State Configured:
        - New validator pubkey
        - SIL1_ADDRESS_WITHDRAWAL_PREFIX credentials

    Output State Verified:
        - Pending deposit added with SIL1 credentials
    """
    amount = spec.MIN_ACTIVATION_BALANCE

    # Create SIL1 withdrawal credentials (0x01 prefix)
    withdrawal_credentials = (
        spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + b"\x00" * 11 + b"\x59" * 20  # 20-byte sil1 address
    )
