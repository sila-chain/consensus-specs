from sil_consensus_specs.test.context import spec_state_test, with_electra_and_later
...
@with_electra_and_later
@spec_state_test
def test_activation_queue_eligibility__min_activation_balance_sil1_creds(spec, state):
    index = 7
    balance = spec.MIN_ACTIVATION_BALANCE
    set_sil1_withdrawal_credential_with_balance(spec, state, index)
    yield from run_test_activation_queue_eligibility(spec, state, index, balance)
