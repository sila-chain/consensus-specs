...
from sil_consensus_specs.test.helpers.churn import get_activation_churn_limit
from sil_consensus_specs.test.helpers.constants import MINIMAL
from sil_consensus_specs.test.helpers.deposits import prepare_pending_deposit
...
def _prepare_sil1_bridge_deprecation(spec, state, sil1_bridge_flags):
    ...
    for index, sil1_bridge in enumerate(sil1_bridge_flags):
        validator_index = validator_index_base + index
        slot = spec.GENESIS_SLOT if sil1_bridge else deposit_request_slot
...
def _check_pending_deposits_induced_new_validators(
    ...
    for index, deposit in enumerate(deposit_requests):
        validator_index = pre_validator_count + len(sil1_bridge_deposits) + index
        validator = state.validators[validator_index]
        assert state.balances[validator_index] == deposit.amount
        assert validator.pubkey == deposit.pubkey
        assert validator.withdrawal_credentials == deposit.withdrawal_credentials
...
def test_process_pending_deposits_not_finalized(spec, state):
    # complete sil1 bridge transition
