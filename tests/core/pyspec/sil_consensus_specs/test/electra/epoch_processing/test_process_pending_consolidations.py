...
def test_basic_pending_consolidation(spec, state):
    ...
    # Set the target withdrawal credential to sil1
    sil1_withdrawal_credential = spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + b"\x00" * 11 + b"\x11" * 20
    state.validators[target_index].withdrawal_credentials = sil1_withdrawal_credential
...
def test_consolidation_not_yet_withdrawable_validator(spec, state):
    ...
    # Set the target to sil1 withdrawal credentials
    sil1_withdrawal_credential = spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + b"\x00" * 11 + b"\x11" * 20
    state.validators[target_index].withdrawal_credentials = sil1_withdrawal_credential
...
def test_skip_consolidation_when_source_slashed(spec, state):
    ...
    sil1_withdrawal_credential = spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + b"\x00" * 11 + b"\x11" * 20
    state.validators[target0_index].withdrawal_credentials = sil1_withdrawal_credential
    state.validators[target1_index].withdrawal_credentials = sil1_withdrawal_credential
...
def test_all_consolidation_cases_together(spec, state):
    ...
    # Set targets withdrawal credentials to sil1
    sil1_withdrawal_credential = spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + b"\x00" * 11 + b"\x11" * 20
    for i in range(4):
        state.validators[target_index[i]].withdrawal_credentials = sil1_withdrawal_credential
...
def test_pending_consolidation_source_balance_less_than_max_effective(spec, state):
    ...
    # Set withdrawable epoch to current epoch to allow processing
    state.validators[source_index].withdrawable_epoch = current_epoch
    # Set source and target withdrawal credential to sil1
    set_sil1_withdrawal_credential_with_balance(spec, state, source_index)
...
def prepare_consolidation_and_state(
    ...
    assert creds_type in ["comp", "sil1"]
...
@with_electra_and_later
@spec_state_test
def test_pending_consolidation_balance_computation_sil1(spec, state):
    instances = []
    for balance_to_eb in ["<", "=", ">"]:
        for eb_to_min_ab, eb_to_max_eb in [("<", "<"), ("=", "=")]:
            instances.append(("sil1", balance_to_eb, eb_to_min_ab, eb_to_max_eb))
