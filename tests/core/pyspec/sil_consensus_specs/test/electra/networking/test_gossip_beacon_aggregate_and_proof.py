...
from sil_consensus_specs.test.helpers.constants import ELECTRA, SILA_FULU, MINIMAL
...
from sil_consensus_specs.test.helpers.state import next_slot
...
def test_gossip_beacon_aggregate_and_proof__accept_same_data_for_disjoint_committees(spec, state):
    """
    [New in Electra:SIP7549] Test that two committee-local aggregates with equal
...
def test_gossip_beacon_aggregate_and_proof__reject_nonzero_data_index(spec, state):
    """
    [New in Electra:SIP7549] Test that an aggregate with ``aggregate.data.index != 0``
    is rejected.
    """
    yield "topic", "meta", "beacon_aggregate_and_proof"
    yield "state", state
    ...
    # Set a non-zero data index (SIP-7549 forbids this).
...
def test_gossip_beacon_aggregate_and_proof__reject_zero_committees(spec, state):
    """
    [New in Electra:SIP7549] Test that an aggregate with no committee bits set
    is rejected.
    """
    yield "topic", "meta", "beacon_aggregate_and_proof"
    yield "state", state
...
def test_gossip_beacon_aggregate_and_proof__reject_multiple_committees(spec, state):
    """
    [New in Electra:SIP7549] Test that an aggregate with more than one committee
    bit set is rejected.
    """
    yield "topic", "meta", "beacon_aggregate_and_proof"
    yield "state", state
