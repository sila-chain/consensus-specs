...
from sil_consensus_specs.test.helpers.constants import ELECTRA, SILA_FULU
...
def test_gossip_beacon_attestation__reject_nonzero_data_index(spec, state):
    """
    [New in Electra:SIP7549] Test that a ``SingleAttestation`` with
...
def test_gossip_beacon_attestation__reject_attester_not_in_committee(spec, state):
    """
    [New in Electra:SIP7549] Test that a ``SingleAttestation`` whose
