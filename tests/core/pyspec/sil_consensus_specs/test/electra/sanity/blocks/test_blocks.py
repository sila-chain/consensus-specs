...
from sil_consensus_specs.test.helpers.bls_to_execution_changes import (
...
from sil_consensus_specs.test.helpers.deposits import (
...
from sil_consensus_specs.test.helpers.voluntary_exits import (
...
def test_basic_el_withdrawal_request(spec, state):
    ...
    validator_index = 0
    address = b"\x22" * 20
    set_sil1_withdrawal_credential_with_balance(spec, state, validator_index, address=address)
...
def test_basic_btec_and_el_withdrawal_request_in_same_block(spec, state):
    ...
    is_execution_address = (
        validator.withdrawal_credentials[:1] == spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX
    )
...
def test_deposit_request_with_same_pubkey_different_withdrawal_credentials(spec, state):
    # signify the sil1 bridge deprecation
    state.deposit_requests_start_index = state.sil1_deposit_index
    ...
    deposit_request_2 = prepare_deposit_request(
        ...
        len(state.validators),
        spec.MIN_ACTIVATION_BALANCE,
        state.sil1_deposit_index + 2,
        signed=True,
        withdrawal_credentials=(spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + b"\x00" * 11 + b"\x11" * 20),
    )
...
def test_withdrawal_and_consolidation_effective_balance_updates(spec, state):
    ...
    # Initialize validator A (0x01 validator with 31.9 SIL)
    ...
    # Initialize validator B (0x02 validator with 64.0 SIL)
    ...
    # Add a pending partial withdrawal for 32 SIL from B
    ...
    assert state.validators[a_index].exit_epoch != spec.FAR_FUTURE_EPOCH
    # Validator B should have an effective balance of 64 SIL
