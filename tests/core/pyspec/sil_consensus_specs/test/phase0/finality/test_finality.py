...
from sil_consensus_specs.test.helpers.attestations import next_epoch_with_attestations
...
def test_finality_rule_3(spec, state):
    ...
    https://github.com/sila-chain/consensus-specs/issues/611#issuecomment-463612892
    """
    # get past first two epochs that finality does not run on
    next_epoch_via_block(spec, state)
    next_epoch_via_block(spec, state)
