...
from sil_consensus_specs.test.helpers.churn import get_exit_churn_limit
from sil_consensus_specs.test.helpers.constants import MINIMAL
from sil_consensus_specs.test.helpers.deposits import mock_deposit
from sil_consensus_specs.test.helpers.epoch_processing import run_epoch_processing_with
from sil_consensus_specs.test.helpers.forks import is_post_electra
from sil_consensus_specs.test.helpers.state import next_epoch, next_slots
...
def test_activation_queue_sorting(spec, state):
    ...
    if is_post_electra(spec):
        # NOTE: SIP-7521 changed how activations are gated
...
def run_test_activation_queue_efficiency(spec, state):
    ...
    for i in range(mock_activations):
        # NOTE: SIP-7251 changes how activations are gated
...
def run_test_activation_queue_activation_and_ejection(spec, state, num_per_status):
    ...
    for validator_index in activation_indices[churn_limit:]:
        validator = state.validators[validator_index]
        assert validator.activation_eligibility_epoch != spec.FAR_FUTURE_EPOCH
        # NOTE: activations are gated differently after SIP-7251
