...
def sil1_init_data(sil1_block_hash, sil1_timestamp):
    yield (
        "sil1",
        {
            "sil1_block_hash": "0x" + sil1_block_hash.hex(),
            "sil1_timestamp": int(sil1_timestamp),
        },
    )
...
def test_initialize_beacon_state_from_sil1(spec):
    ...
    yield from sil1_init_data(sil1_block_hash, sil1_timestamp)
    ...
    assert state.sil1_data.deposit_root == deposit_root
    assert state.sil1_data.deposit_count == deposit_count
    assert state.sil1_data.block_hash == sil1_block_hash
...
def test_initialize_beacon_state_some_small_balances(spec):
    ...
    assert state.sil1_data.deposit_root == deposit_root
    assert state.sil1_data.deposit_count == len(deposits)
    assert state.sil1_data.block_hash == sil1_block_hash
...
def test_initialize_beacon_state_random_invalid_genesis(spec):
    ...
    sil1_block_hash = b"\x14" * 32
    sil1_timestamp = spec.config.MIN_GENESIS_TIME + 1
...
def test_initialize_beacon_state_random_valid_genesis(spec):
    ...
    sil1_block_hash = b"\x15" * 32
    sil1_timestamp = spec.config.MIN_GENESIS_TIME + 2
