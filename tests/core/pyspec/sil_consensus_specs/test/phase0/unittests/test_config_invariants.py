...
from sil_consensus_specs.test.helpers.constants import UINT64_MAX
...
def test_validators(spec, state):
    ...
    # Note: can be less if you assume stricters bounds on validator set based on total SIL supply
...
def test_incentives(spec, state):
    # Ensure no SIL is minted in slash_validator
