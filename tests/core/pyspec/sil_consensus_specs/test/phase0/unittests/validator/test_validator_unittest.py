...
from sil_consensus_specs.test.helpers.block import build_empty_block
from sil_consensus_specs.test.helpers.constants import SILA_FULU, PHASE0
from sil_consensus_specs.test.helpers.keys import privkeys, pubkeys
from sil_consensus_specs.test.helpers.state import next_epoch
from sil_consensus_specs.utils import bls
from sil_consensus_specs.utils.ssz.ssz_typing import Bitlist
...
def run_is_candidate_block(spec, sil1_block, period_start, success=True):
    assert success == spec.is_candidate_block(sil1_block, period_start)


def get_min_new_period_epochs(spec):
    return (
        (spec.config.SECONDS_PER_SIL1_BLOCK * spec.config.SIL1_FOLLOW_DISTANCE * 2)  # to seconds
        * 1000
...
def test_is_candidate_block(spec, state):
    distance_duration = spec.config.SECONDS_PER_SIL1_BLOCK * spec.config.SIL1_FOLLOW_DISTANCE
...
def test_get_sil1_vote_consensus_vote(spec, state):
    ...
    votes_length = spec.get_current_epoch(state) % spec.EPOCHS_PER_SIL1_VOTING_PERIOD
...
@with_all_phases_from_to(from_phase=PHASE0, to_phase=SILA_FULU)
@spec_state_test_with_matching_config
