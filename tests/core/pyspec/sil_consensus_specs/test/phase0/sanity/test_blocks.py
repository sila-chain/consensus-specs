...
from sil_consensus_specs.test.helpers.attestations import get_valid_attestation
from sil_consensus_specs.test.helpers.attester_slashings import (
...
from sil_consensus_specs.test.helpers.constants import SILA_FULU, MINIMAL, PHASE0
from sil_consensus_specs.test.helpers.deposits import prepare_state_and_deposit
...
from sil_consensus_specs.test.helpers.keys import pubkeys
from sil_consensus_specs.test.helpers.multi_operations import (
    run_slash_and_exit,
    run_test_full_random_operations,
)
from sil_consensus_specs.test.helpers.proposer_slashings import (
...
from sil_consensus_specs.test.helpers.sync_committee import (
    compute_committee_indices,
    compute_sync_committee_participant_reward_and_penalty,
)
from sil_consensus_specs.test.helpers.voluntary_exits import prepare_signed_exits
...
@with_presets(
    [MINIMAL],
    reason="sila_mainnet config leads to larger validator set than limit of public/private keys pre-generated",
)
@spec_test
@with_custom_state(
...
def test_empty_epoch_transition_not_finalizing(spec, state):
    if spec.SLOTS_PER_EPOCH > 8:
        return dump_skipping_message(
            "Skip sila_mainnet config for saving time."
...
def test_high_proposer_index(spec, state):
    ...
    for i in range(len(state.validators) // 3):
        if is_post_sila_fulu(spec) and i in state.proposer_lookahead:
            continue
...
def test_sil1_data_votes_consensus(spec, state):
    ...
    for i in range(voting_period_slots):
        ...
        block.body.sil1_data.block_hash = b if i * 2 > voting_period_slots else a
        signed_block = state_transition_and_sign_block(spec, state, block)
        blocks.append(signed_block)

    assert len(state.sil1_data_votes) == voting_period_slots
    assert state.sil1_data.block_hash == a

    # transition to next sil1 voting period
    block = build_empty_block_for_next_slot(spec, state)
    block.body.sil1_data.block_hash = c
    signed_block = state_transition_and_sign_block(spec, state, block)
    blocks.append(signed_block)
    ...
    assert len(state.sil1_data_votes) == 1
    assert state.sil1_data_votes[0].block_hash == c
