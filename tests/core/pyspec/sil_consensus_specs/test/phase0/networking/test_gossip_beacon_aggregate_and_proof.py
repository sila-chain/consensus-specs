...
@with_phases([PHASE0, ALTAIR, BELLATRIX, CAPELLA, SILA_DENEB, ELECTRA, SILA_FULU])
@spec_state_test
...
def test_gossip_beacon_aggregate_and_proof__ignore_same_data_root_without_superset(spec, state):
    ...
    if is_post_electra(spec):
        # Electra ``Attestation`` carries an SIP-7549 aggregation bitlist sized for
...
@with_phases([PHASE0, ALTAIR, BELLATRIX, CAPELLA, SILA_DENEB, ELECTRA, SILA_FULU])
@with_presets([SILA_MAINNET], reason="minimal preset has committees < 16, so everyone is an aggregator")
@spec_test
@with_custom_state(
