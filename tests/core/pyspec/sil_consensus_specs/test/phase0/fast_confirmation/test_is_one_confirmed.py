...
from sil_consensus_specs.test.helpers.fast_confirmation import (
    FCRTest,
    Slashing,
)
...
def test_is_one_confirmed_passes_large_validator_supporting(spec, state):
    """
    Test with 2048 SIL validator supporting confirmed block
    """
    fcr = FCRTest(spec, seed=1)
    store, fcr_store = fcr.initialize(state)
...
def test_is_one_confirmed_fails_large_validator_not_supporting(spec, state):
    """
    Test with 2048 SIL validator not supporting confirmed block
    """
    fcr = FCRTest(spec, seed=1)
    store, fcr_store = fcr.initialize(state)
...
def test_is_one_confirmed_fails_large_validator_slashed(spec, state):
    """
    Test with 2048 SIL validator supporting a confirmed block got slashed,
...
def test_is_one_confirmed_passes_large_validator_slashed(spec, state):
    """
    Test with 66 SIL validator supporting a confirmed block but slashed.
    66 SIL would be slightly higher than 25% of the average committee weight,
...
def test_is_one_confirmed_passes_large_validator_voting_in_empty_slot(spec, state):
    """
    Test with 2048 SIL validator voting during empty slot,
