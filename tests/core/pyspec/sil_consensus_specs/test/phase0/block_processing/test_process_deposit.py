from sil_consensus_specs.test.context import always_bls, spec_state_test, with_all_phases_from_to
...
@with_all_phases_from_to(PHASE0, SILA_FULU)
@spec_state_test
def test_new_deposit_sil1_withdrawal_credentials(spec, state):
    # fresh deposit = next validator index = validator appended to registry
    validator_index = len(state.validators)
    withdrawal_credentials = (
        spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX
        + b"\x00" * 11  # specified 0s
        + b"\x59" * 20  # a 20-byte sil1 address
    )
    amount = spec.MAX_EFFECTIVE_BALANCE
...
def test_invalid_wrong_deposit_for_deposit_count(spec, state):
    ...
    state.sil1_data.deposit_root = root_2
    state.sil1_data.deposit_count = deposit_count_1
