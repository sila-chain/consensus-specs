...
from sil_consensus_specs.test.utils.yield_generator import MultiPhaseResult, vector_test
from sil_consensus_specs.utils import bls
from sil_consensus_specs.utils.ckzg_utils import apply_ckzg_to_spec
...
with_sila_deneb_and_later = with_all_phases_from(SILA_DENEB)
with_electra_and_later = with_all_phases_from(ELECTRA)
with_sila_fulu_and_later = with_all_phases_from(SILA_FULU, all_phases=ALLOWED_TEST_RUNNER_FORKS)
...
with_sip8025_and_later = with_all_phases_from(SIP8025, all_phases=ALLOWED_TEST_RUNNER_FORKS)
with_sip8148_and_later = with_all_phases_from(SIP8148, all_phases=ALLOWED_TEST_RUNNER_FORKS)
...
def get_copy_of_spec(spec):
    ...
    module_path = f"sil_consensus_specs.{fork}.{preset}"
...
def with_fork_metas(fork_metas: Sequence[ForkMeta]):
    ...
    `spec`: the version of the sil2 spec corresponding to `pre_fork_name`.
    `post_spec`: the version of the sil2 spec corresponding to `post_fork_name`.
