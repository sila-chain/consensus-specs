...
from sil_consensus_specs.test.helpers.blob import get_block_with_blob_and_sidecars
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__ok(spec, state):
    """
    Similar to test_simple_blob_data, but in SilaPeerDAS version that is from SilaFulu onwards.
...
def run_on_block_sila_peerdas_invalid_test(spec, state, fn):
    """
    Run a invalid SilaPeerDAS on_block test with a sidecars mutation function.
    """
    rng = Random(1234)
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__not_available(spec, state):
    """
    Test is_data_available throws an exception when not enough columns are sampled.
    """
    yield from run_on_block_sila_peerdas_invalid_test(
        spec,
        state,
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_zero_blobs(spec, state):
    """
    ...
    yield from run_on_block_sila_peerdas_invalid_test(spec, state, invalid_zero_blobs)


@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_index_1(spec, state):
    """
    Test invalid index in sidecars for negative SilaPeerDAS on_block test.
    """
    ...
    yield from run_on_block_sila_peerdas_invalid_test(spec, state, invalid_index)


@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_index_2(spec, state):
    """
    Test invalid index in sidecars for negative SilaPeerDAS on_block test.
    """
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_mismatch_len_column_1(spec, state):
    """
    Test mismatch length in column for negative SilaPeerDAS on_block test.
    """
    ...
    yield from run_on_block_sila_peerdas_invalid_test(spec, state, invalid_mismatch_len_column)


@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_mismatch_len_column_2(spec, state):
    """
    Test mismatch length in column for negative SilaPeerDAS on_block test.
    """
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_mismatch_len_kzg_commitments_1(spec, state):
    """
    Test mismatch length in kzg_commitments for negative SilaPeerDAS on_block test.
    """
    ...
    yield from run_on_block_sila_peerdas_invalid_test(spec, state, invalid_mismatch_len_kzg_commitments)


@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_mismatch_len_kzg_commitments_2(spec, state):
    """
    Test mismatch length in kzg_commitments for negative SilaPeerDAS on_block test.
    """
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_mismatch_len_kzg_proofs_1(spec, state):
    """
    Test mismatch length in kzg_proofs for negative SilaPeerDAS on_block test.
    """
    ...
    yield from run_on_block_sila_peerdas_invalid_test(spec, state, invalid_mismatch_len_kzg_proofs)


@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_mismatch_len_kzg_proofs_2(spec, state):
    """
    Test mismatch length in kzg_proofs for negative SilaPeerDAS on_block test.
    """
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_wrong_column_1(spec, state):
    """
    Test wrong column for negative SilaPeerDAS on_block test.
    """
    ...
    yield from run_on_block_sila_peerdas_invalid_test(spec, state, invalid_wrong_column)


@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_wrong_column_2(spec, state):
    """
    Test wrong column for negative SilaPeerDAS on_block test.
    """
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_wrong_commitment_1(spec, state):
    """
    Test wrong commitment for negative SilaPeerDAS on_block test.
    """
    ...
    yield from run_on_block_sila_peerdas_invalid_test(spec, state, invalid_wrong_commitment)


@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_wrong_commitment_2(spec, state):
    """
    Test wrong commitment for negative SilaPeerDAS on_block test.
    """
    alt_sidecars = get_alt_sidecars(spec, state)
...
@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_wrong_proof_1(spec, state):
    """
    Test wrong proof for negative SilaPeerDAS on_block test.
    """
    alt_sidecars = get_alt_sidecars(spec, state)
    ...
    yield from run_on_block_sila_peerdas_invalid_test(spec, state, invalid_wrong_proof)


@with_all_phases_from_to(SILA_FULU, GLOAS)
@spec_state_test
def test_on_block_sila_peerdas__invalid_wrong_proof_2(spec, state):
    """
    Test wrong proof for negative SilaPeerDAS on_block test.
    """
    alt_sidecars = get_alt_sidecars(spec, state)
