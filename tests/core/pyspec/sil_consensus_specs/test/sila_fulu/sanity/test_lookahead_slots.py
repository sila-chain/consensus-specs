...
def test_effective_decrease_balance_updates_lookahead(spec, state):
    """
    Test that effective balance updates change the proposer lookahead with SIP-7917.
    """
    ...
    if not is_post_sila_fulu(spec):
        # Pre-SIP-7917, effective balance changes changes the next epoch's lookahead
        assert next_epoch_lookahead != actual_lookahead
    else:
        # Post-SIP-7917, effective balance changes do not change the next epoch's lookahead
        assert next_epoch_lookahead == actual_lookahead
