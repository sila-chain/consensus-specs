...
def run_test_with_randao_setup_epochs(spec, state, randao_setup_epochs):
    if spec.fork == ELECTRA:
        # Pre-SIP-7917, effective balance changes due to attestation rewards
        # changes the next epoch's lookahead
        expect_lookahead_changed = True
    else:
        # Post-SIP-7917, effective balance changes due to attestation rewards
...
@with_phases(phases=[ELECTRA, SILA_FULU])
@spec_state_test
