...
@with_phases(phases=[BELLATRIX], other_phases=[CAPELLA, SILA_DENEB])
@spec_test
@with_config_overrides(
    {
        "CAPELLA_FORK_EPOCH": 1 * 8,  # SyncCommitteePeriod 1
        "SILA_DENEB_FORK_EPOCH": 2 * 8,  # SyncCommitteePeriod 2
    },
)
@with_state
@with_matching_spec_config(emitted_fork=SILA_DENEB)
@with_presets([MINIMAL], reason="too slow")
def test_capella_sila_deneb_reorg_aligned(spec, phases, state):
    yield from run_lc_data_collection_test_multi_fork(spec, phases, state, CAPELLA, SILA_DENEB)
...
@with_config_overrides(
    {
        ...
        "SILA_DENEB_FORK_EPOCH": 3 * 8 + 4,  # SyncCommitteePeriod 3 (+ 4 epochs)
    },
)
@with_state
@with_matching_spec_config(emitted_fork=SILA_DENEB)
@with_presets([MINIMAL], reason="too slow")
def test_capella_sila_deneb_reorg_unaligned(spec, phases, state):
    yield from run_lc_data_collection_test_multi_fork(spec, phases, state, CAPELLA, SILA_DENEB)
