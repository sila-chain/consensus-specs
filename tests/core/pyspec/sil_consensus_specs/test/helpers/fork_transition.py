...
from sil_consensus_specs.test.helpers.consolidations import (
...
def _state_transition_and_sign_block_at_slot(spec, state, sync_aggregate=None, operation_dict=None):
    ...
    p.s. we can't just pass `body` and assign it because randao_reveal and sil1_data was set in `build_empty_block`
...
def run_transition_with_operation(
    ...
    if is_slashing_operation:
        ...
        if operation_type == OperationType.PROPOSER_SLASHING:
            ...
            if is_at_fork and spec.fork == SILA_DENEB:
                # NOTE: attestation format changes between SilaDeneb and Electra
    ...
    def _check_state():
        if operation_type == OperationType.PROPOSER_SLASHING:
            ...
            validator = state.validators[selected_validator_index]
            assert validator.withdrawal_credentials[:1] == spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX
        elif operation_type == OperationType.DEPOSIT_REQUEST:
