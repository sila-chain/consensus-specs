...
def fill_aggregate_attestation(
    ...
    if is_post_electra(spec):
        ...
        attestation.aggregation_bits = get_empty_sip7549_aggregation_bits(
    ...
    for i in range(len(beacon_committee)):
        if is_post_electra(spec):
            offset = get_sip7549_aggregation_bits_offset(
...
def get_valid_attestation_at_slot(
    ...
    Note: this SIP supports dense packing of on-chain aggregates so we can just return a single `Attestation`.
    """
    assert is_post_electra(spec)
...
def get_empty_sip7549_aggregation_bits(spec, state, committee_bits, slot):
...
def get_sip7549_aggregation_bits_offset(spec, state, slot, committee_bits, committee_index):
    """
