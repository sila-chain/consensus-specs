...
from sil_consensus_specs.sila_fulu.sila_mainnet import DataColumnSidecar
from sil_consensus_specs.test.context import expect_assertion_error
from sil_consensus_specs.test.exceptions import BlockNotFoundException
from sil_consensus_specs.test.helpers.attestations import (
    next_epoch_with_attestations,
    next_slots_with_attestations,
    state_transition_with_full_block,
)
from sil_consensus_specs.test.helpers.block import build_empty_block_for_next_slot
from sil_consensus_specs.test.helpers.forks import is_post_sila_fulu, is_post_gloas
from sil_consensus_specs.test.helpers.state import state_transition_and_sign_block
...
def with_blob_data(spec, blob_data: BlobData, func):
    ...
    elif is_post_sila_fulu(spec):
        if blob_data.sidecars is None:
            raise ValueError("blob_data.sidecars must be provided post-SilaFulu")
        yield from with_blob_data_sila_fulu(spec, blob_data, func)
    else:
        if blob_data.proofs is None:
            raise ValueError("blob_data.proofs must be provided pre-SilaFulu")
        yield from with_blob_data_sila_deneb(spec, blob_data, func)


def with_blob_data_sila_deneb(spec, blob_data: BlobData, func):
    """
...
def with_blob_data_sila_fulu(spec, blob_data: BlobData, func):
    """
...
def add_block(
    ...
    if blob_data is not None:
        assert blob_data.is_pre_sila_fulu() or blob_data.is_post_sila_fulu(), "Integrity fail blob_data"

        if blob_data.is_pre_sila_fulu():
        ...
        if blob_data.is_post_sila_fulu():
