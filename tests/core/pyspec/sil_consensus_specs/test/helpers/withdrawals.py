...
def set_validator_fully_withdrawable(spec, state, index, withdrawable_epoch=None):
    ...
    if validator.withdrawal_credentials[0:1] == spec.BLS_WITHDRAWAL_PREFIX:
        validator.withdrawal_credentials = (
            spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + validator.withdrawal_credentials[1:]
        )
...
def set_sil1_withdrawal_credential_with_balance(
    ...
    validator = state.validators[index]
    validator.withdrawal_credentials = spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX + b"\x00" * 11 + address
...
def sample_withdrawal_indices(spec, state, rng, num_full_withdrawals, num_partial_withdrawals):
    ...
    # Get proposers in lookahead (if post-sila_fulu)
    proposer_set = set(state.proposer_lookahead) if is_post_sila_fulu(spec) else set()
