...
from sil_consensus_specs.test.helpers.fork_transition import (
...
def needs_upgrade_to_sila_deneb(spec, new_spec):
    return is_post_sila_deneb(new_spec) and not is_post_sila_deneb(spec)
...
def upgrade_lc_header_to_new_spec(spec, new_spec, data, phases):
    ...
    if needs_upgrade_to_sila_deneb(spec, new_spec):
        upgraded = phases[SILA_DENEB].upgrade_lc_header_to_sila_deneb(upgraded)
        check_lc_header_equal(spec, phases[SILA_DENEB], data, upgraded)
...
def upgrade_lc_bootstrap_to_new_spec(spec, new_spec, data, phases):
    ...
    if needs_upgrade_to_sila_deneb(spec, new_spec):
        upgraded = phases[SILA_DENEB].upgrade_lc_bootstrap_to_sila_deneb(upgraded)
        check_lc_bootstrap_equal(spec, phases[SILA_DENEB], data, upgraded)
...
def upgrade_lc_update_to_new_spec(spec, new_spec, data, phases):
    ...
    if needs_upgrade_to_sila_deneb(spec, new_spec):
        upgraded = phases[SILA_DENEB].upgrade_lc_update_to_sila_deneb(upgraded)
        check_lc_update_equal(spec, phases[SILA_DENEB], data, upgraded)
...
def upgrade_lc_finality_update_to_new_spec(spec, new_spec, data, phases):
    ...
    if needs_upgrade_to_sila_deneb(spec, new_spec):
        upgraded = phases[SILA_DENEB].upgrade_lc_finality_update_to_sila_deneb(upgraded)
        check_lc_finality_update_equal(spec, phases[SILA_DENEB], data, upgraded)
...
def upgrade_lc_store_to_new_spec(spec, new_spec, data, phases):
    ...
    if needs_upgrade_to_sila_deneb(spec, new_spec):
        upgraded = phases[SILA_DENEB].upgrade_lc_store_to_sila_deneb(upgraded)
        check_lc_store_equal(spec, phases[SILA_DENEB], data, upgraded)
