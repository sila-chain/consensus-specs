...
from sil_consensus_specs.test.helpers.forks import is_post_altair, is_post_electra
...
from sil_consensus_specs.test.helpers.state import get_balance
from sil_consensus_specs.utils import bls
from sil_consensus_specs.utils.merkle_minimal import calc_merkle_tree_from_leaves, get_merkle_proof
from sil_consensus_specs.utils.ssz.ssz_impl import hash_tree_root
from sil_consensus_specs.utils.ssz.ssz_typing import List
from tests.core.pyspec.sil_consensus_specs.test.helpers.churn import get_activation_churn_limit
...
def prepare_state_and_deposit(
    ...
    state.sil1_data.deposit_root = root
    state.sil1_data.deposit_count = len(deposit_data_list)
    return deposit
...
def prepare_builder_deposit_request(
    ...
    if withdrawal_credentials is None:
        # Version zero followed by an sil1 address derived from the pubkey
...
def run_deposit_processing(spec, state, deposit, validator_index, valid=True, effective=True):
    ...
    assert state.sil1_deposit_index == state.sil1_data.deposit_count
...
def run_pending_deposit_applying(spec, state, pending_deposit, validator_index, effective=True):
    ...
    # ensure the transition from sil1 bridge is complete
