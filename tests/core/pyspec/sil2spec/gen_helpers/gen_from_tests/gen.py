...
from sil2spec.test.helpers.constants import ALL_PRESETS, TESTGEN_FORKS
from sil2spec.test.helpers.typing import PresetBaseName, SpecForkName
...
def get_expected_modules(module, absolute=False):
    ...
    def _cached_walk_packages() -> list[ModuleInfo]:
        """Walk the packages in the sil2spec module. Cache the result for future calls."""
        global CACHED_WALK_PACKAGES

        if CACHED_WALK_PACKAGES is None:
            sil2spec = import_module("sil2spec")
            path = sil2spec.__path__
            prefix = sil2spec.__name__ + "."
