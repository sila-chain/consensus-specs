...
def get_post_gloas_genesis_execution_payload_header(spec, slot, sil1_block_hash):
...
def get_sample_genesis_execution_payload_header(spec, slot, sil1_block_hash=None):
    ...
    if is_post_gloas(spec):
        return get_post_gloas_genesis_execution_payload_header(spec, slot, sil1_block_hash)
    payload_header = spec.ExecutionPayloadHeader(
        parent_hash=b"\x30" * 32,
        fee_recipient=b"\x42" * 20,
        state_root=b"\x20" * 32,
...
def create_genesis_state(spec, validator_balances, activation_threshold):
    ...
    if is_post_gloas(spec):
        ...
        genesis_block_body.signed_execution_payload_bid.message.block_hash = sil1_block_hash
