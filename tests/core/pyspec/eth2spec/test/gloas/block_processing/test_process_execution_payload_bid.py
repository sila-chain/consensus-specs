...
from sil2spec.test.helpers.block import build_empty_block_for_next_slot
...
def test_process_execution_payload_bid_valid_builder(spec, state):
    ...
    value = spec.Gwei(1000000)  # 0.001 SIL
...
def test_process_execution_payload_bid_excess_balance(spec, state):
    """
    Test builder with excess balance (2048.25 SIL) can submit bid for 2016.25 SIL
    ...
    excess_balance = spec.Gwei(2048250000000)  # 2048.25 SIL in Gwei
    bid_value = spec.Gwei(2016250000000)  # 2016.25 SIL in Gwei
...
def test_process_execution_payload_bid_insufficient_balance_with_pending_payments(spec, state):
    ...
    balance = spec.MIN_ACTIVATION_BALANCE + spec.Gwei(1000)  # 32 SIL + 1000 gwei
...
def test_process_execution_payload_bid_sufficient_balance_with_pending_payments(spec, state):
    ...
    # Set up scenario: balance=2000, bid=600, existing_pending=500, min_activation=32SIL
    # Total needed: 600 + 500 + 32000000000 = ~32.0011 SIL < 2000 SIL (should pass)
    balance = spec.Gwei(2000000000000)  # 2000 SIL
    bid_amount = spec.Gwei(600)
    existing_pending = spec.Gwei(500)
...
def test_process_execution_payload_bid_sufficient_balance_with_pending_withdrawals(spec, state):
    ...
    # Set up scenario: balance=2000, bid=600, existing_withdrawal=500, min_activation=32SIL
    # Total needed: 600 + 500 + 32000000000 = ~32.0011 SIL < 2000 SIL (should pass)
    balance = spec.Gwei(2000000000000)  # 2000 SIL
    bid_amount = spec.Gwei(600)
    existing_withdrawal = spec.Gwei(500)
